...
class EmailService {
    ...
    async sendTaskAssignedEmail(userId, taskId, assignedByUserId) {
        try {
            // Get user data
            ...
            const assignedBy = await prisma_1.prisma.user.findUnique({
    ...
    async sendCommentMentionEmail(userId, commentId, mentionedByUserId) {
        // Get the comment first to avoid circular reference
        const commentPromise = prisma_1.prisma.comment.findUnique({
            where: { id: commentId },
            select: {
                id: true,
                content: true,
        ...
        const mentionedByPromise = prisma_1.prisma.user.findUnique({
  ...
  `;
    ...
    async sendProjectInviteEmail(userId, projectId, invitedByUserId) {
        const [user, project, invitedBy] = await Promise.all([
            ...
            prisma_1.prisma.user.findUnique({ where: { id: invitedByUserId } })
        ]);
        if (!user || !project || !invitedBy)
            return false;
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
