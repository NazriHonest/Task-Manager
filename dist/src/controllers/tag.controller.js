...
exports.searchTags = exports.getPopularTags = exports.updateTagTasks = exports.getTagStats = exports.bulkDeleteTags = exports.deleteTag = exports.updateTag = exports.createTag = exports.getTag = exports.getTags = void 0;
    ...
    try {
        ...
        if (query.includeTaskCount) {
            const tagsWithCounts = await Promise.all(tags.map(async (tag) => {
                const taskCount = await prisma_1.prisma.taskTag.count({
                    where: { tagId: tag.id }
                });
                return {
                    ...tag,
        ...
        if (query.withTasks) {
            const tagsWithTasks = await Promise.all(tags.map(async (tag) => {
                const taskTags = await prisma_1.prisma.taskTag.findMany({
                    where: { tagId: tag.id },
                    take: 5,
                    include: {
                        task: {
                ...
                taskTags.sort((a, b) => new Date(b.task.createdAt).getTime() - new Date(a.task.createdAt).getTime());
                return {
                    ...tag,
...
const updateTagTasks = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        ...
        if (action === 'assign') {
            ...
            for (const assignment of assignments) {
                try {
                    await prisma_1.prisma.taskTag.create({
                        data: assignment
                    });
                }
                catch (error) {
            ...
            await prisma_1.prisma.taskTag.deleteMany({
                where: {
                    tagId: id,
                    taskId: { in: taskIds }
                }
...
exports.updateTagTasks = updateTagTasks;
