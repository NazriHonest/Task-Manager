...
exports.getPerformanceMetrics = exports.getProjectAnalytics = exports.getUserActivity = exports.getProductivityAnalytics = exports.getDashboardStats = void 0;
...
const logActivity = async (userId, action, entityId, entityType, metadata) => {
    try {
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const [totalTasks, completedTasks, pendingTasks, inProgressTasks, overdueTasks, totalProjects, activeProjects, totalComments, totalAttachments, priorityStats, recentActivities] = await Promise.all([
            // Total tasks
            prisma_1.prisma.task.count({ where: baseWhere }),
    ...
    try {
        ...
        tasks.forEach(task => {
            ...
            if (productivityData[dateKey].tasksCreated > 0) {
                productivityData[dateKey].completionRate = Math.round((productivityData[dateKey].tasksCompleted / productivityData[dateKey].tasksCreated) * 100);
            }
        });
        const chartData = Object.values(productivityData).sort((a, b) => a.date.localeCompare(b.date));
        // Get completed tasks - FIX: Use string literal
        const completedTasks = await prisma_1.prisma.task.findMany({
            where: {
                ...baseWhere,
                status: 'COMPLETED',
    ...
    try {
        ...
        const tasksByMember = tasks.reduce((acc, task) => {
            ...
            if (task.status === 'IN_PROGRESS')
                acc[memberId].inProgress++;
            if (task.status === 'PENDING')
                acc[memberId].pending++;
            return acc;
        }, {});
        const timelineData = {};
        tasks.forEach(task => {
        ...
        const recentActivity = await prisma_1.prisma.comment.findMany({
            where: {
                task: { projectId },
                createdAt: { gte: start, lte: end }
            },
        ...
        const overdueTasks = tasks.filter(t => t.dueDate && t.dueDate < new Date() &&
            ['PENDING', 'IN_PROGRESS'].includes(t.status)).length;
